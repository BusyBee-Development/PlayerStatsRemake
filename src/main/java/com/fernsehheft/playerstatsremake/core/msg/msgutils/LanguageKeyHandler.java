package com.fernsehheft.playerstatsremake.core.msg.msgutils;

import com.fernsehheft.playerstatsremake.core.Main;
import com.fernsehheft.playerstatsremake.core.utils.EnumHandler;
import com.fernsehheft.playerstatsremake.core.utils.YamlFileHandler;
import com.fernsehheft.playerstatsremake.api.enums.Unit;
...
public final class LanguageKeyHandler extends YamlFileHandler {
    ...
    private String getStatKeyTranslationFromFile(String statKey) {
        ...
        // Fix for issue #175: new MC statistics (e.g. happy_ghast_one_cm) may not exist
        // in the language file yet. Return a prettified fallback instead of null to prevent NPE.
        String translation = super.getFileConfiguration().getString(realKey);
        if (translation == null) {
            // Extract just the last segment of the key (e.g. "happy_ghast" from "stat.minecraft.happy_ghast_one_cm")
            String[] parts = realKey.split("\\.");
            return StringUtils.prettify(parts[parts.length - 1]);
        }
        return translation;
    }
